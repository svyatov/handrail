...
up to 4 KB each, so a 30-line range runs 15-40 KB. Each bullet or table row opens with
its field or bold term: find it with ``grep -n '^[-|] [`*]*<term>' docs/spec.md`` and
read those lines alone. `GLOSSARY.md` defines the vocabulary.
