...
// UserDir is Claude Code's user-level directory, which is also how sync detects
// the harness: the CLI creates it on first run, so its absence means no install
// worth writing config for.
func UserDir() string {
...
// Installed reports whether Claude Code has run on this machine.
func Installed() bool {
	dir := UserDir()
	if dir == "" {
		return false
...
// SettingsPath is the one file sync writes: user-level, never project-level.
...
// Install puts exactly one hook entry per canonical event into the user-level
// settings, invoking bin. It reports how many entries it wrote and whether the
// file needed changing. Every other key in the file is left exactly as it was:
// these are the user's settings, and handrail is one tenant among several.
func Install(bin string) (entries int, changed bool, err error) {
	path := SettingsPath()
	if path == "" {
	...
	for _, event := range events {
		groups := prune(hooks[event], event)
		...
		hooks[event] = append(groups, map[string]any{
			"hooks": []any{map[string]any{
				"type":    "command",
				"command": shellQuote(bin) + " hook " + Name + " " + event,
			}},
		})
	}
...
func ours(command, event string) bool {
	tail, found := strings.CutSuffix(command, " hook "+Name+" "+event)
	if !found {
		return false
	}
...
func prune(groups any, event string) []any {
	list, _ := groups.([]any)
	...
	for _, g := range list {
		...
		for _, h := range inner {
			...
			if ok {
				if cmd, _ := hook["command"].(string); ours(cmd, event) {
					continue
				}
			}
...
// Degradations reports where Claude Code weakens a rule's action. Sync and
// doctor print this; the runtime hot path stays quiet about it.
func Degradations(rules []*rule.Rule) []Degradation {
	var out []Degradation
	for _, r := range rules {
		if !r.Enabled || r.ShadowedBy != "" {
			continue
		}
		if r.Action == "block" && !canBlock(r.Event) {
			out = append(out, Degradation{
				Rule: r.Name, From: "block", To: "warn",
				Reason: "Claude Code has no denial to give on " + r.Event,
			})
		}
		...
		if !canInject(r.Event) {
			out = append(out, Degradation{
				...
				Reason: "Claude Code discards hook output on " + r.Event + ", so the message goes to the user, not the agent",
			})
		}
	}
	return out
...
// Quirks are the harness behaviours a user should know about but handrail
// cannot change. Reported next to the degradations, for the same reason.
func Quirks() []string {
	return []string{
