...
- All translation knowledge lives in the Go CLI per adapter, kept current by binary releases: the mechanism, the file its entries are pasted into, and the spelling are one declaration on the Adapter, so a harness added without them offers no advice rather than silently offering none. The skill contains zero harness-specific pattern knowledge and only relays.
- **Paste target**: the user-level file the entry goes in (`<claude-dir>/settings.json`, `<codex-dir>/rules/default.rules`), resolved through the same directory sync detects and the same relocation variables. `advise` never gates on whether the harness is installed, but it does gate on there being a directory to name: with no home directory and no relocation variable set, the harness offers no advice rather than a path that points nowhere.
